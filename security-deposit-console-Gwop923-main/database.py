...
class Database:
    ...
    def __init__(self, path):
        ...
        with sqlite3.connect(self.db_file) as conn:
            ...
            if not cur.fetchone():
                ...
                conn.commit()
                with open(_path / 'accounts.csv.bak', encoding="utf8") as infile:
                    next(infile)
                    for line in infile:
                        account_num, owner, balance = line.strip().split(',')
                        cur.execute(
                            f"INSERT INTO accounts (account_num, owner, balance) VALUES ({account_num}, '{owner}', {balance});")

            cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='users';")
            if not cur.fetchone():
                cur.execute(
                    "CREATE TABLE users (username TEXT NOT NULL PRIMARY KEY, account_num INTEGER, role TEXT, password TEXT, salt TEXT);")
                conn.commit()
                with open(_path / 'users.csv.bak', encoding="utf8") as infile:
                    next(infile)
                    for line in infile:
    ...
    def create_account(self, acct: Account) -> Account:
        ...
        with sqlite3.connect(self.db_file) as conn:
            cur = conn.cursor()
            cur.execute(f'INSERT INTO accounts VALUES ({acct.account_num}, "{acct.owner_name}", {acct.balance}, NULL);')
            if cur.rowcount == 0:
                return None
        return acct
    ...
    def search_by_id(self, acct_num: int) -> Account:
        ...
        with sqlite3.connect(self.db_file) as conn:
            cur = conn.cursor()
            cur.execute(f"SELECT * FROM accounts WHERE account_num = {acct_num};")
    ...
    def update_account(self, acct: Account) -> Optional[Account]:
        ...
        with sqlite3.connect(self.db_file) as conn:
            cur = conn.cursor()
            sql = f'UPDATE accounts SET owner = "{acct.owner_name}", balance = {acct.balance}, notes = "{acct.notes}" WHERE account_num = {acct.account_num};'
